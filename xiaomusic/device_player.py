...
class XiaoMusicDevice:
    ...
    async def _play_internal(self, name="", search_key=""):
        ...
        # 如果name为空但search_key不为空，使用search_key作为name
        if not name and search_key:
            # 检查是否是"播放音乐"、"来点音乐"等指令
            if search_key in ["播放音乐", "来点音乐", "播放歌曲", "来点歌曲"]:
                # 对于这些指令，使用空字符串作为name，这样会随机播放歌曲
                name = ""
            else:
                name = search_key

        self.log.info(
            f"play_internal. search_key:{search_key} name:{name}"
        )
    ...
    async def _play_next(self):
        ...
        if name == "":
            ...
            return
        await self._play_internal(name)
    ...
    async def _playmusic(self, name):
        ...
        # 打印Audio对象信息
        music = self.xiaomusic.music_library.all_music.get(name)
        if music:
            self.log.info(f"Audio对象类型: {type(music)}")
            if hasattr(music, 'name'):
                self.log.info(f"Audio对象信息: name={music.name}, duration={music.duration}, id={music.id}, album={music.album}")
            else:
    ...
    async def check_replay(self):
        ...
        if self.is_playing:
            ...
            self.log.info(
                f"不会继续播放歌曲. isplaying:{self.is_playing}"
            )
    ...
    async def _text_to_speech_edge_tts(self, value):
        ...
        try:
            ...
            # 计算 TTS 时长（基于文本长度）
            duration = min(8, int(len(value) / 3))
    ...
    async def group_player_play(self, url, name=""):
        ...
        # 处理 Emby 音频流 URL 的代理
        proxied_url = url
        if url and "emby/Audio/" in url and "stream?Container=" in url:
            # 提取音频 ID 和容器类型
            import re
            match = re.search(r"/Audio/([^/]+)/stream\?Container=([^&]+)", url)
            if match:
                audio_id = match.group(1)
                container = match.group(2)
                # 使用当前服务器作为代理
                current_host = f"{self.config.hostname}:{self.config.public_port}"
                proxied_url = f"{current_host}/xiaoemby/proxy/emby/audio/{audio_id}.{container}"
                self.log.info(f"Emby音频流URL已转换为代理URL: {url} -> {proxied_url}")
        
        device_id_list = self.xiaomusic.device_manager.get_group_device_id_list(
            self.group_name
        )
        tasks = [
            self.play_one_url(device_id, proxied_url, name) for device_id in device_id_list
        ]
        results = await asyncio.gather(*tasks)
        self.log.info(f"group_player_play {proxied_url} {device_id_list} {results}")
        return results
    ...
    async def set_next_music_timeout(self, sec):
        ...
        async def _do_next():
            ...
            try:
                ...
                # 执行完毕后清除定时器引用
                if self._next_timer and self._next_timer.done():
                    self._next_timer = None
    ...
    async def play_music_list(self, list_name, music_name):
        ...
        await self._play_internal(music_name)
    ...
    def find_cur_playlist(self, name):
        ...
        for list_name, play_list in music_list.items():
            if (list_name not in ["全部", "所有歌曲"]) and (
                name in play_list
            ):
                return list_name
