...
from xiaomusic.api.models import Did, DidCmd, DidVolume
...
@router.post("/test_cmd")
async def test_cmd(data: Dict[str, str]):
    """测试指令匹配"""
    ...
    cmd = data.get("cmd", "")
    log.info(f"test_cmd. cmd:{cmd}")
    
    # 模拟后端指令匹配逻辑
    for pattern, action in command_action_dict.items():
        match = re.match(pattern, cmd)
        if match:
            return {"ret": "OK", "action": action, "pattern": pattern, "groups": match.groupdict()}
    
    return {"ret": "OK", "action": None, "pattern": None, "groups": None}
