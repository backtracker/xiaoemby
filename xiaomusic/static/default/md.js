...
$.get("/getsetting", function (data, status) {
  ...
  if (did == "web_device") {
    ...
    // 其他按钮启用（播放模式、上一曲、播放、下一曲、停止、音量、设置）
...
function toggleTestResult() {
  const $resultComponent = $("#test-result-component");
  const isVisible = $resultComponent.is(":visible");
  if (isVisible) {
...
function copyTestResult() {
    // 获取结果元素
    const commandElement = document.getElementById('result-command');
    const patternElement = document.getElementById('result-pattern');
    const actionElement = document.getElementById('result-action');
    const groupsElement = document.getElementById('result-groups');
    
    // 检查元素是否存在
    if (!commandElement || !patternElement || !actionElement || !groupsElement) {
        console.error('无法找到结果元素');
        return;
    }
    
    // 提取文本内容
    const commandText = commandElement.textContent.trim();
    const patternText = patternElement.textContent.trim();
    const actionText = actionElement.textContent.trim();
    const groupsText = groupsElement.textContent.trim();
...
function showCopyFeedback() {
    ...
    // 设置文本并显示
    feedbackElement.textContent = '复制成功！';
...
function testCmdMatch() {
  var cmd = $("#test-cmd-input").val();
  if (!cmd || cmd.trim() === "") {
    alert("请输入要测试的指令");
    return;
  }
  $.ajax({
    ...
    success: (response) => {
      console.log("测试指令匹配结果:", response);
      
      if (response.ret === "OK") {
        if (response.action) {
          // 更新弹窗内容 - 使用更安全的方式填充文本
          const commandText = "测试指令：" + cmd;
          const patternText = "匹配正则：" + response.pattern;
          const actionText = "匹配动作：" + response.action;
          const groupsText = "分组信息：" + JSON.stringify(response.groups, null, 1);
          ...
          // 添加新的文本内容
          commandElement.appendChild(document.createTextNode(commandText));
          patternElement.appendChild(document.createTextNode(patternText));
          actionElement.appendChild(document.createTextNode(actionText));
          groupsElement.appendChild(document.createTextNode(groupsText));
    ...
    error: (error) => {
      console.log("测试指令匹配失败:", error);
      alert("测试指令匹配失败，请检查网络连接");
    },
  });
}
...
function refreshlist() {
  $.ajax({
    type: "POST",
    url: "/xiaoemby/music/refreshlist",
    contentType: "application/json; charset=utf-8",
    data: JSON.stringify({}),
    success: () => {
...
function togglePullAsk() {
  ...
  $.get("/getsetting", function (data, status) {
    ...
    $.ajax({
      type: "POST",
      url: "/xiaoemby/system/modifiysetting",
      contentType: "application/json; charset=utf-8",
      data: JSON.stringify({
